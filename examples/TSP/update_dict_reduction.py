...
- performing (1/r) partial update of the dictionary,
loading batches of b/r in between
- performing 1 full update of the dictionary, loading batches of size b in
 between

 b was set using synthetic_batch_size experiement
"""
...
def run(n_jobs=1, n_epochs=10):
    ...
    global_exp = dict(n_components=100, alpha=1,
                      ...
                      AB_agg='full')
    ref_batch_size = 200
    exps = [dict(batch_size=int(ref_batch_size / reduction),
                 reduction=reduction)
            for reduction in np.linspace(1, 10, 5)]
...
def plot(output_dir):
    with open(join(output_dir, 'compare.json'), 'r+') as f:
        ...
        fig, axes = plt.subplots(n_redundancies, 2, figsize=(14, 12),
                                 squeeze=False,
                                 sharey='row')
        n_reduction = len(df['reduction'].unique())
        sns_colormap = sns.color_palette('Blues_r', n_reduction,
                                         )
        colormap = {}
        z_index = {}
        for ii, reduction in enumerate(np.sort(df['reduction'].unique())):
            colormap[reduction] = sns_colormap[ii]
            z_index[reduction] = ii
        for i, (redundancy, this_df) in enumerate(df.groupby('redundancy')):
            lim = [1000, -1000]
            for idx, line in this_df.iterrows():
                axes[i, 0].plot(line['times'][1:], line['obj'][1:],
                                label='reduction %s' % (line['reduction']),
                                marker='o', color=colormap[line['reduction']],
                                zorder=z_index[line['reduction']])
                axes[i, 1].plot(line['iter'][1:], line['obj'][1:],
                                label='reduction %s' % (line['reduction']),
                                marker='o', color=colormap[line['reduction']],
                                zorder=z_index[line['reduction']])
        ...
        axes[0, 1].legend(ncol=1, bbox_to_anchor=(1, 1), loc='upper left')
        plt.savefig(join(output_dir, 'compare.pdf'))


if __name__ == '__main__':
    output_dir = expanduser('~/output/modl/update_dict_reduction')
