...
def plot_profiling_data(name):
    ...
    algorithm_name = {'full': 'No code\nsubsampling',
                      'tsp gram': 'Code\nsubsampling'}
    for i, algorithm in enumerate(['full', 'tsp gram']):
        for exp in algorithm_exps[algorithm]:
            ...
            labels = np.array(
                ['',
                 ...
                 '$\\boldsymbol{\\alpha}_t$', '$\\mathbf{G}_t$',
                 '$\\mathbf{D}_t$'])
        ...
        if i == 1:
            ...
            axes[i, -1].xaxis.set_label_coords(axes[i, -1].get_xlim()[1] * 3,
                                               0, transform=trans)
    ...
    axes[1, 0].annotate('Computation', xytext=(0, -8),
                        ...
                        xy=(-.2, -.2), xycoords='axes fraction', va='top', ha='left')
...
def plot_method_comparison(xlim, ylim, ylim_zoom, xlim_zoom, name):
    ...
    for i, algorithm in enumerate(algorithm_keys):
        ...
        for exp in sorted(exps,
            ...
            # mean_diff_time = np.mean(diff_time)
    ...
    axes[0].annotate("Zoom", xycoords='data',
                     xy=(
                         rect_0[0] * 0.9, rect_0[1] + 0.5 * rect_len[1]),
                     va='center', ha='right')
    ...
    l = axes[1].legend(handles[n_red - 1::n_red],
                       ...
                       bbox_to_anchor=(1.45, 0.65), loc='upper center',
                       ncol=1,
