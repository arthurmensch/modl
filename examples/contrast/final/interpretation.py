...
for i, condition in enumerate(conditions):
    ...
    fig.subplots_adjust(top=0.99, left=0.14, right=.99, bottom=0.23, hspace=0,
                        wspace=0.1)
    ...
    for j, img in enumerate(imgs):
        ...
        plot_stat_map(this_img, axes=axes[j], figure=fig, display_mode='z',
                      ...
                      cut_coords=cut_coords, colorbar=False, annotate=False,
                      draw_cross=False)
        axes[j].annotate(exps[j], xy=(.5, 0.01),
                         fontsize=7,
                         # bbox=bbox_props,
                         xycoords='axes fraction', ha='center', va='top')
    axes[0].annotate('L', fontsize=7,
                     xy=(.2, .9),
                     xycoords='axes fraction', ha='center', va='center', )
    axes[0].annotate('R', fontsize=7,
                     xy=(.8, .9),
                     xycoords='axes fraction', ha='center', va='center', )
    axes[0].annotate('%s\nz=%imm' % (conditions_name[condition],
                                      cut_coords[0]),
                     fontsize=7,
                     xy=(-.51, .9),
