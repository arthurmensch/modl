...
def run(penalty_list,
    ...
    seed_list = check_random_state(_seed).randint(np.iinfo(np.uint32).max,
                                                  size=n_seeds)
    param_grid = ParameterGrid({'penalty': penalty_list,
                                'dropout': dropout_list,
                                'latent_dim': latent_dim_list,
                                'alpha': alpha_list,
                                'activation': activation_list,
                                'seed': seed_list})
    single_dataset_parameter_grid = ParameterGrid(
        {'datasets': ['archi'],
         ...
         'activation': activation_list,
         'seed': seed_list})
    non_factored_parameter_grid = ParameterGrid({'datasets': ['archi'],
                                                 'factored': [False],
                                                 'penalty': penalty_list,
                                                 'alpha': alpha_list,
                                                 'seed': seed_list})
    param_grid = chain(param_grid, single_dataset_parameter_grid,
                       non_factored_parameter_grid)
