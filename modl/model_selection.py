...
from sklearn.externals.joblib import Parallel, logger
...
class MemGridSearchCV(GridSearchCV):
    def __init__(self, estimator, param_grid, scoring=None, fit_params=None,
                 ...
                 return_train_score=True, keep_best=True):
        ...
        **fit_params : dict of string -> object
        ...
        out = Parallel(
            ...
            pre_dispatch=pre_dispatch
                                  ...
                                  return_estimator=self.keep_best,
        ...
        if self.keep_best:
            best_per_split = \
                np.array(test_scores, dtype=np.float64).reshape(n_candidates,
                                                                n_splits)
            best_per_split = np.argmin(best_per_split, axis=0)
            best_per_split += np.arange(0, (n_splits - 1) * n_candidates + 1,
                                        n_candidates, dtype='int')
            self.best_estimators_ = [estimators[i] for i in best_per_split]
