...
class ScipyOpt(Optimizer):
    ...
    def __init__(self, model, x, y, nb_epoch=500, method='L-BFGS-B', verbose=1,
                 **kwargs):
        ...
        for layer in self.model.layers:
            ...
            for param in layer.get_weights():
                ...
                layer_weights.append(
                    np.asarray(theta[idx:(idx + plen)].reshape(param.shape),
                               dtype=np.float32))
                idx += plen
    ...
    def get_train_fn(self, params, loss):
        ...
        fn = K.function(inputs=[], outputs=outs,
                        givens={self.model.model.inputs[0]: self.x,
                                ...
                                self.model.model.sample_weights[0]: np.ones(
                                    (self.x.shape[0],), dtype=np.float32),
            ...
            if self.verbose > 0:
                if self.epoch_idx % 1 == 0:
                    try:
                        ...
                        pass
            grads = np.asarray(self.flatten_grads(cost_grads[1:]),
                               dtype=np.float64)
            if self.verbose > 1:
